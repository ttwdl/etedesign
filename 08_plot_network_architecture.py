...
3. 36 个滤光片把 151 维光谱压成 36 维测量（训练时还会加噪声）；
4. MLP 解码器（36→1024→512→151，输出经 Softplus 保证非负）把测量重建回光谱；
...
def draw_decoder_nodes(ax) -> None:
    """在解码器框里画 36 → 1024 → 512 → 151 的四层全连接示意（只是示意，圆点数量不代表真实维度）。"""

    x_cols = [9.15, 9.70, 10.25, 10.80]          # 四列的横坐标
    y_nodes = {
        "in":  [3.78, 3.62, 3.46, 3.30],          # 36
        "h1":  [3.84, 3.68, 3.52, 3.36, 3.20],    # 1024
        "h2":  [3.84, 3.68, 3.52, 3.36, 3.20],    # 512
    ...
    # 每列下面标维度
    for xi, label in zip(x_cols, ["36", "1024", "512", "151"]):
...
def main() -> None:
    ...
    ax.text(6.0, 5.42,
            "输入是按位深缩放后的光谱强度；物理编码器给出 36 通道测量(训练时加噪)；MLP 输出重建 151 维光谱",
    ...
    add_box(ax, (5.35, 3.45), (1.25, 1.55), "可微 TMM",
            "由层结构计算\nT_m(λ, α)\n输出透过谱\nshape: [36,151]", "#e8daef", title_size=12, body_size=9)

    add_box(ax, (7.05, 3.45), (1.45, 1.55), "光电测量 + 噪声",
            "y_m = Σ S(λ)T_m(λ)\n积分求和\n训练时加噪声\nshape: [B,36]", "#d5f5e3", title_size=12, body_size=9)
    ...
    add_arrow(ax, (8.50, 4.25), (8.90, 4.25), "36维测量")
    ...
    ax.text(6.0, 0.18,
            "当前模型不是 CNN, 也不学空间邻域；它学的是“单条光谱经过 36 个物理滤光片后如何重建”。",
